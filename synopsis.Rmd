...
filso_kz_filt <- filso_kz %>% 
  ...
  filter(kz_hobo > 0) %>% 
    mutate(year = year(date),
           ...
           wnd_dir_bin = case_when(between(wnd_dir, 45, 45+90*1) ~ "E",
                                   between(wnd_dir, 45+90*1, 45+90*2) ~ "S",
                                   between(wnd_dir, 45+90*2, 45+90*3) ~ "W",
                                   TRUE ~ "N"),
           wnd_dir_bin = factor(wnd_dir_bin))
...
### Model and model reduction with site as random and corGaus correlation structure

Fit initial model:

Three-way interaction with maximum wind speed, wind direction binned (N, S, E, W) and season (spring, summer and autumn) and a fixed effect of year. 

```{r, warning=FALSE, message=FALSE}

form <- formula(kz_hobo ~ wnd_max*wnd_dir_bin*season+year)

#No residual correlation term
#m <- lme(form, data = model_df, random = ~1|site, method = "ML")

#Corgaus term
m0 <- lme(form, data = model_df, random = ~1|site, method = "ML",
          correlation = corGaus(form = ~ as.numeric(date)|site, nugget = FALSE))

```

Reduce model using drop1:

```{r}
drop1(m0, test = "Chisq")
```

Update model and drop again:
...
```{r}
plot(Variogram(m1, form = ~ as.numeric(date)|site, data = model_df, nugget = FALSE))
```
...
```{r}
plot(ACF(m1, retype="normalized"))
```

Raw output from summary:
